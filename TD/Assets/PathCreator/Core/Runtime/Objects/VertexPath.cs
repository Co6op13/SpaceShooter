...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        public Quaternion GetRotationAtDistance2d(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Loop)
        {
            float t = dst / length;
            return GetRotation2d(t, endOfPathInstruction);
        }
        ...
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
            Vector3 direction = (localPoints[data.nextIndex] - localPoints[data.previousIndex]);
